...
Now let's join the two datasets! I will first perform a left join.

```{r}
tmp <- ortho_df %>%
  filter(the_date < outcome_cutoff)

ortho_months <-split(tmp, tmp$binding_month)
```

If all goes well the below code should work.
...
I will finish by joining the data as promised. The outcome data still needs to be made though.
...
One of the biggest bottlenecks is missing data.

I hope that this "tutorial" helps someone and perhaps someone will do supervised machine learning with the process data.
